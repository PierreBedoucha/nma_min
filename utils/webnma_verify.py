# DESCRIPTION: Since WEBnma (V2 and V3) APIs generates many text
# outputs, which can include both numbers and words, use the 'verify'
# function to verify them easily: (line by line) numbers should be
# equal, and words should be identical
...
def equal_nums(l1, l2, atol=1e-05):
    if len(l1) != len(l2):
        return False
    ...
    return np.allclose(np.array(l1), np.array(l2), atol=atol, equal_nan=True)
...
def verify(file1, file2, ignore_text=False):
    ...
    for i, (l1,l2) in enumerate(zip(c1,c2)):
        l1_ls = l1.strip().split()
        if is_number(l1_ls[0]):
            l2_ls = l2.strip().split()
            if not equal_nums(l1_ls, l2_ls):                
                diff_lines.append(i)
        else:
            if not ignore_text:
                if not l1 == l2:
